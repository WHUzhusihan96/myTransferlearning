function [acc,acc_ite,A] = BDA(X_src,Y_src,X_tar,Y_tar,options)
    ...
    X = [X_src',X_tar'];% 输入矩阵X
	X = X*diag(sparse(1./sqrt(sum(X.^2))));% 归一化
	[m,n] = size(X);% m为特征数，n为样本量
	ns = size(X_src,1);% 源域样本量
	nt = size(X_tar,1);% 目标域样本量
	e = [1/ns*ones(ns,1);-1/nt*ones(nt,1)];% 中心向量
	C = length(unique(Y_src));% 种类数

    %% Centering matrix H
	H = eye(n) - 1/n * ones(n,n);% 中心矩阵
	%%% M0矩阵
	...
	for i = 1 : T
        ...
        if mu == 1 %这里不太懂为什么，不加该部分的报错为：矩阵为奇异值、接近奇异值或缩放错误
            mu = 0.999;
        end
        M = (1 - mu) * M + mu * N;% BDA的MMD矩阵
        M = M / norm(M,'fro');
