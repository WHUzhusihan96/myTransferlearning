...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
	...
	for i = 1 : T
        ...
        %对Z进行归一化，并得到源域和目标域分别的值
        Zs = Z(:,1:size(X_src,1));
        Zt = Z(:,size(X_src,1)+1:end);
        %SVM
        %model = libsvmtrain(Y_src, Zs','-c 100');
        %[~, acc, ~] = libsvmpredict(Y_tar, Zt', model);
        %fprintf('JDA+SVM=%0.4f\n',acc);
        %acc_ite = [acc_ite;acc];
        %KNN
        knn_model = fitcknn(Zs',Y_src,'NumNeighbors',1);%Xs_new,Ys
        Y_tar_pseudo = knn_model.predict(Zt');%Xt_new
...
function [Z,A] = JDA_core(X_src,Y_src,X_tar,Y_tar_pseudo,options)
	...
	X = [X_src',X_tar'];%800*(ns+nt)
	X = X*diag(sparse(1./sqrt(sum(X.^2))));%做归一化
	[m,n] = size(X);%m=800,n=ns+nt
	ns = size(X_src,1);
	nt = size(X_tar,1);
	e = [1/ns*ones(ns,1);-1/nt*ones(nt,1)];%单域的L矩阵
	C = length(unique(Y_src));%得到标签类数（分类类数）
	...
	%%% Mc %%%
	N = 0;
	if ~isempty(Y_tar_pseudo) && length(Y_tar_pseudo)==nt%看有没有伪标签（也就是是不是第一次），且伪标签个数等于nt，确保没有问题
		for c = reshape(unique(Y_src),1,C)%对类别数进行循环
			e = zeros(n,1);%下面是求联合分布的单域的 L矩阵
			e(Y_src==c) = 1 / length(find(Y_src==c));%类别为c的对应索引的值的计算，详情看论文，源域
			e(ns+find(Y_tar_pseudo==c)) = -1 / length(find(Y_tar_pseudo==c));%类别为c的对应索引值得计算，目标域
			e(isinf(e)) = 0;%isinf(e)得到0和1（无穷为1，其余为0），然后将1的逻辑赋值为0.即防止出现无穷大的数
			N = N + e*e';%Mc计算方法与上面类似。
		end
	end
